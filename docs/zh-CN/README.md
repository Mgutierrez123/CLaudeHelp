...
**搞定！** 你现在可以使用 53 个智能体、200 项技能和 92 个命令了。

***

## 跨平台支持

此插件现已针对 Claude Code 完全支持 **Windows、macOS 和 Linux**。所有钩子和脚本都已用 Node.js 重写，以实现最大兼容性。
...
不能。此分支专注于 **仅 Claude Code** —— `claude`（用户目录，`~/.claude/`）和 `claude-project`（项目内，`./.claude/`）是唯一支持的安装目标。为使安装器和技能/智能体范围专注于单一工具，已移除对其他平台（Cursor、Codex、OpenCode、Antigravity、JoyCode、CodeBuddy、Qwen CLI、Zed、Gemini CLI）的支持。
