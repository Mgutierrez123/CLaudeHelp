...
Плагин теперь полностью поддерживает **Windows, macOS и Linux** для Claude Code. Все хуки и скрипты переписаны на Node.js для максимальной совместимости.
...
Нет. Этот форк ограничен только **Claude Code** — `claude` (домашняя директория, `~/.claude/`) и `claude-project` (в проекте, `./.claude/`) являются единственными целями установки. Поддержка других сред (Cursor, Codex, OpenCode, Antigravity, JoyCode, CodeBuddy, Qwen CLI, Zed, Gemini CLI) была удалена, чтобы установщик и набор skills/агентов оставались сосредоточены на одной среде.
</details>
