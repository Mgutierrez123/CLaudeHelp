...
/project-init --target claude-project
...
2. 既存のプロジェクトガイダンスを保持。`CLAUDE.md`または`.claude/settings.local.json`が既に存在する場合、内容を検査し上書きではなくマージ/追記計画を提案。
...
1. ターゲットを特定。ユーザーが`claude-project`（プロジェクト単位、`./.claude/`）を要求しない限りデフォルトは`claude`（ホーム、`~/.claude/`）。
