...
function runTests() {
  ...
  if (test('fails when README project tree agent count drifts', () => {
    const testDir = createTestDir();
    const {
      readmePath,
      agentsPath,
    ...
    assert.strictEqual(result.code, 1, 'Should fail when README project tree agent count drifts');
    assert.ok(
      (result.stdout + result.stderr).includes('README.md project tree'),
      'Should mention the README project tree mismatch'
    );
    cleanupTestDir(testDir);
  })) passed++; else failed++;
