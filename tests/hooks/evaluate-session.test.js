...
function runTests() {
  ...
  if (test('uses custom min_session_length from config file', () => {
    ...
    const configDir = path.join(testDir, 'skills', 'continuous-learning-v2');
    ...
    // __dirname = scripts/hooks, so config = repo_root/skills/continuous-learning-v2/config.json
  ...
  if (test('falls back to defaults when config file contains invalid JSON', () => {
    ...
    // where __dirname = scripts/hooks/ → config = repo_root/skills/continuous-learning-v2/config.json
